...
export default function DashboardPage() {
  ...
  const fetchCars = async () => {
    try {
      ...
      const uniqueBrands: string[] = [...new Set(data.map((car: Car) => car.brand))];
        ...
        <Stack>
          <Group grow>
            <TextInput
              ...
              placeholder="Ex: Onix"
              value={filters.model}
              onChange={(e) => setFilters({ ...filters, model: e.currentTarget.value })}
            />
            <Select
              label="Marca"
              placeholder="Todas"
            ...
            <NumberInput
              ...
              placeholder="Ex: 2020"
              value={filters.year}
              onChange={(value) => setFilters({ ...filters, year: value?.toString() || "" })}
            />
            <NumberInput
              label="Preço mín."
              value={filters.priceMin}
              onChange={(value) => setFilters({ ...filters, priceMin: value?.toString() || "" })}
            />
            <NumberInput
              label="Preço máx."
              value={filters.priceMax}
              onChange={(value) => setFilters({ ...filters, priceMax: value?.toString() || "" })}
            />
          </Group>
        </Stack>
      </Card>
            ...
            <Table.Tbody>
              {cars.map((car) => (
                <Table.Tr key={car._id}>
                  ...
                  <Table.Td>R$ {Number(car.price).toLocaleString("pt-BR")}</Table.Td>
                </Table.Tr>
              ))}
            </Table.Tbody>
          </Table>
